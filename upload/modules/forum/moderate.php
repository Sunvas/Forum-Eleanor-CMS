...
class ForumModerate extends Forum
{	public function DeletePost($ids,$data=array())
	...
	*/
	...
	/*
		Восстановка форумных lp
		$fids=array(
			'IDforum'=>array('lang1','lang2'..)
		)
	*/
	public function RepairForums($fids,$tids=false)
	{		$R=Eleanor::$Db->Query('SELECT `id`,`language` FROM `'.$this->config['fl'].'` WHERE `id`'.Eleanor::$Db->In(array_keys($fids)).($tids ? ' AND `lp_id`'.Eleanor::$Db->In($tids) : ''));
		while($a=$R->fetch_assoc())
	...
	*/
	public function MoveTopic($ids,$to,$data=array())
	{		if(!$ids)
		...
		$data+=array(
			...
			'language'=>Language::$main,#Язык форума, куда перемещаем темы
